...
public class FormatSplitter {
    ...
    // Note: This probably supports more formats than MC's formatter does, except things like local translation for I18n
    // seems to
    ...
    public static List<Component> split(String text) {
        ...
        if (start < text.length()) {
            if (start == 0) {
                ...
                // Note: This assumes every thing between two {} is valid, instead of bothering to check against the set
                // that java + forge declare
    ...
    private static List<Component> splitMessageFormatInternal(String text) {
                    ...
                    if (leftBrackets == 0) {
                        ...
                        if (component == null) {
                            if (secondBracket != -1) {
                                ...
                                // We use subtract one here so when it is incremented at the end of the loop, it starts
                                // in the right place
                                i = secondBracket - 1;
                            } else {
                                // If we only have a depth of one, and it is not a valid message format, then we just
                                // add it as a raw string
                                components.add(new TextComponent(piece));
                            }
                        } else {
                            // The string we found represents a valid MessageFormat so add the component for it to our
                            // components
        ...
        if (leftBrackets == 0) {
            ...
            if (secondBracket != -1) {
                ...
                // If we don't have a closing bracket, and we didn't have more brackets at some point, add what we have
                // as raw text
    ...
    public static class MessageFormatComponent extends FormatComponent {
         ...
         *
         * @return A {@link MessageFormatComponent} representing the given contents, or {@code null} if the contents do
        ...
        private static MessageFormatComponent fromContents(String contents) {
            ...
            if (length < 3 || contents.charAt(0) != '{' || contents.charAt(length - 1) != '}') {
                // If we don't have at least one digit between the two brackets, or we don't start and end with a
                // bracket
            ...
            switch (trimmedFormatType) {
                ...
                case "date", "time" -> {
                    if (formatStyle != null && !formatStyle.equals("short") && !formatStyle.equals("medium") && !formatStyle.equals("long") && !formatStyle.equals("full")) {
                        ...
                        try {
                            ...
                            // If it is not a valid SimpleDateFormat then it is not a valid format overall, so return
                            // null
                            return null;
                        }
                    }
                ...
                case "choice" -> {
                    ...
                    // Choice is only valid when it has a SubformatPattern, so we return null if we don't have a
                    // formatStyle
                    try {
                        new ChoiceFormat(formatStyle);
                    } catch (IllegalArgumentException e) {
                        // If it is not a valid ChoiceFormat to begin with then it is not a valid format overall, so
                        // return null
                        return null;
                    }
                    isChoice = true;
