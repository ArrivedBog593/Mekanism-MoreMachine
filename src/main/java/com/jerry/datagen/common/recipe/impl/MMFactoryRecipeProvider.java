...
class MMFactoryRecipeProvider implements ISubRecipeProvider {
    ...
    public void addRecipes(Consumer<FinishedRecipe> consumer) {
        ...
        for (MMFactoryType type : MMEnumUtils.MM_FACTORY_TYPES) {
            BlockRegistryObject<MMBlockFactory<?>, MMItemBlockFactory> basicFactory = MMBlocks.getMMFactory(FactoryTier.BASIC, type);
            BlockRegistryObject<MMBlockFactory<?>, MMItemBlockFactory> advancedFactory = MMBlocks.getMMFactory(FactoryTier.ADVANCED, type);
            BlockRegistryObject<MMBlockFactory<?>, MMItemBlockFactory> eliteFactory = MMBlocks.getMMFactory(FactoryTier.ELITE, type);
            ...
            addFactoryRecipe(consumer, ultimatePath, MMBlocks.getMMFactory(FactoryTier.ULTIMATE, type), eliteFactory, Tags.Items.GEMS_DIAMOND, MekanismTags.Items.ALLOYS_ATOMIC, MekanismTags.Items.CIRCUITS_ULTIMATE);
        }
    }

    private void addFactoryRecipe(Consumer<FinishedRecipe> consumer, String basePath, BlockRegistryObject<MMBlockFactory<?>, ?> factory, IItemProvider toUpgrade,
        ...
        MoreMachineDataShapedRecipeBuilder.shapedRecipe(factory)
              ...
              .build(consumer, Mekmm.rl(basePath + Objects.requireNonNull(Attribute.get(factory, MMAttributeFactoryType.class)).getMMFactoryType().getRegistryNameComponent()));
    }
}