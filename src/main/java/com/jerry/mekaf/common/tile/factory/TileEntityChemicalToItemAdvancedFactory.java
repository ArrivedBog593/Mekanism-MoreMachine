...
public abstract class TileEntityChemicalToItemAdvancedFactory<RECIPE extends MekanismRecipe<?>> extends TileEntityAdvancedFactoryBase<RECIPE>{
    ...
    protected void addSlots(InventorySlotHelper builder, IContentsListener listener, IContentsListener updateSortingListener) {
        outputSlot = new IInventorySlot[tier.processes];
        ...
        for (int i = 0; i < tier.processes; i++) {
            ...
            builder.addSlot(outputSlot[i]);
            itemOutputHandlers[i] = OutputHelper.getOutputHandler(outputSlot[i], CachedRecipe.OperationTracker.RecipeError.NOT_ENOUGH_OUTPUT_SPACE);
        }
    }
    ...
    protected static class CIRecipeProcessInfo<RECIPE extends MekanismRecipe<?>> {
        ...
        @Nullable
        private ToIntBiFunction<CIRecipeProcessInfo<RECIPE>, TileEntityChemicalToItemAdvancedFactory<RECIPE>> lazyMinPerSlot;
        private Object item;
        private RECIPE recipe;
        private long minPerSlot = 1;
        private long totalCount;

        public long getMinPerSlot(TileEntityChemicalToItemAdvancedFactory<RECIPE> factory) {
