...
public abstract class TileEntityItemToMergedFactory<RECIPE extends MekanismRecipe> extends TileEntityAdvancedFactoryBase<RECIPE> {
    ...
    protected ItemToFourMergedProcessInfo[] processInfoSlots;
    ...
    protected TileEntityItemToMergedFactory(IBlockProvider blockProvider, BlockPos pos, BlockState state, List<CachedRecipe.OperationTracker.RecipeError> errorTypes, Set<CachedRecipe.OperationTracker.RecipeError> globalErrorTypes) {
        ...
        for (int i = 0; i < tier.processes; i++) {
            processInfoSlots[i] = new ItemToFourMergedProcessInfo(i, inputSlot[i], outputTank[i]);
        }

        for (ItemToFourMergedProcessInfo info : processInfoSlots) {
    ...
    protected void sortInventoryOrTank() {
        ...
        List<ItemToFourMergedProcessInfo> emptyProcesses = new ArrayList<>();
        for (ItemToFourMergedProcessInfo processInfo : processInfoSlots) {
        ...
        for (Map.Entry<HashedItem, ItemToFourMergedRecipeProcessInfo> entry : processes.entrySet()) {
            ...
            if (recipeProcessInfo.lazyMinPerSlot == null) {
                ...
                recipeProcessInfo.lazyMinPerSlot = () -> {
                    ...
                    ItemToFourMergedProcessInfo processInfo = recipeProcessInfo.processes.get(0);
    ...
    private void addEmptySlotsAsTargets(Map<HashedItem, ItemToFourMergedRecipeProcessInfo> processes, List<ItemToFourMergedProcessInfo> emptyProcesses) {
        for (Map.Entry<HashedItem, ItemToFourMergedRecipeProcessInfo> entry : processes.entrySet()) {
            ItemToFourMergedRecipeProcessInfo recipeProcessInfo = entry.getValue();
            ...
            int added = 0;
            List<ItemToFourMergedProcessInfo> toRemove = new ArrayList<>();
            for (ItemToFourMergedProcessInfo emptyProcess : emptyProcesses) {
    ...
    private void distributeItems(Map<HashedItem, ItemToFourMergedRecipeProcessInfo> processes) {
        for (Map.Entry<HashedItem, ItemToFourMergedRecipeProcessInfo> entry : processes.entrySet()) {
            ...
            for (int i = 0; i < processCount; i++) {
                ItemToFourMergedProcessInfo processInfo = recipeProcessInfo.processes.get(i);
    ...
    public record ItemToFourMergedProcessInfo(int process, @NotNull AdvancedFactoryInputInventorySlot inputSlot,
                                              @NotNull MergedChemicalTank outputTank) {
    }

    public static class ItemToFourMergedRecipeProcessInfo {

        private final List<ItemToFourMergedProcessInfo> processes = new ArrayList<>();
        @Nullable
