...
//    protected IInputHandler<@NotNull ItemStack>[] inputHandlers;
    ...
    protected void sortInventoryOrTank() {
        Map<ItemStack, CORecipeProcessInfo> processes = ItemStackMap.createTypeAndTagMap();
        List<COProcessInfo> emptyProcesses = new ArrayList<>();
        for (COProcessInfo processInfo : processInfoSlots) {
            IInventorySlot inputSlot = processInfo.inputSlot();
            if (inputSlot.isEmpty()) {
                emptyProcesses.add(processInfo);
            } else {
                ItemStack inputStack = inputSlot.getStack();
                CORecipeProcessInfo recipeProcessInfo = processes.computeIfAbsent(inputStack, i -> new CORecipeProcessInfo());
                ...
                if (recipeProcessInfo.lazyMinPerSlot == null && !CommonWorldTickHandler.flushTagAndRecipeCaches) {
                    ...
                    CachedRecipe<ItemStackToChemicalRecipe> cachedRecipe = getCachedRecipe(processInfo.process());
        ...
        for (Map.Entry<ItemStack, CORecipeProcessInfo> entry : processes.entrySet()) {
            CORecipeProcessInfo recipeProcessInfo = entry.getValue();
            if (recipeProcessInfo.lazyMinPerSlot == null) {
                ...
                recipeProcessInfo.lazyMinPerSlot = (info, factory) -> {
                    ...
                    COProcessInfo processInfo = info.processes.getFirst();
    ...
    private void addEmptySlotsAsTargets(Map<ItemStack, CORecipeProcessInfo> processes, List<COProcessInfo> emptyProcesses) {
        for (Map.Entry<ItemStack, CORecipeProcessInfo> entry : processes.entrySet()) {
            CORecipeProcessInfo recipeProcessInfo = entry.getValue();
            ...
            int added = 0;
            List<COProcessInfo> toRemove = new ArrayList<>();
            for (COProcessInfo emptyProcess : emptyProcesses) {
    ...
    private void distributeItems(Map<ItemStack, CORecipeProcessInfo> processes) {
        for (Map.Entry<ItemStack, CORecipeProcessInfo> entry : processes.entrySet()) {
            CORecipeProcessInfo recipeProcessInfo = entry.getValue();
            int processCount = recipeProcessInfo.processes.size();
            if (processCount == 1) {
            ...
            for (int i = 0; i < processCount; i++) {
                COProcessInfo processInfo = recipeProcessInfo.processes.get(i);
    ...
    public record COProcessInfo(int process, @NotNull AdvancedFactoryInputInventorySlot inputSlot, @NotNull IChemicalTank outputTank) {
    }
